...
Every team member must configure their local environment precisely to support the cross-platform mobile framework and backend services[cite: 3].
...
2. Download and install Docker Desktop. This is mandatory for hosting our local Supabase instance and containerizing the external OpenCV engine[cite: 3].
3. Open your terminal and clone this repository using the command `git clone https://github.com/NaesCode/Paws_and_Found`[cite: 1].
...
Our architecture relies on Supabase for the PostgreSQL database and a Python container for the artificial intelligence image matching[cite: 3].

1. Launch Docker Desktop and ensure the Docker engine is actively running in the background.
2. Open your terminal in the project root and execute `supabase start` to spin up the PostgreSQL database, Storage buckets, and Edge Functions locally[cite: 3].
3. Change directories into the backend AI folder and run `docker-compose up -d` to boot the OpenCV image processing container in detached mode[cite: 3].
4. Confirm that the PostGIS spatial extensions and database schemas are properly loaded before attempting to render the localized community map[cite: 3].
